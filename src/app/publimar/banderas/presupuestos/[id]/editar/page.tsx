...
export default function EditarPresupuestoPage({ params }: { params: { id: string } }) {
    ...
    const addItemToQuote = () => {
      ...
      const price = Number(
        selectedVariant ? selectedVariant.price : selectedProduct.price || 0
      );
      const discountAmount = (price * itemDiscount) / 100;
      const priceAfterDiscount = price - discountAmount;
      const subtotal = priceAfterDiscount * itemQuantity;
      // Calculamos el taxAmount para el item
      const itemTaxAmount = subtotal * (Number(formData.taxRate) / 100);
      ...
      const newItem: QuoteItem = {
        ...
        subtotal: subtotal,
        taxAmount: itemTaxAmount, // Agregamos el taxAmount calculado
        notes: itemNotes,
      };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (Object.values(quoteData).some((value) => value === undefined)) {
          console.error("Datos inválidos:", quoteData);
          toast.error("Error: Hay datos inválidos en el presupuesto");
          return;
        }
